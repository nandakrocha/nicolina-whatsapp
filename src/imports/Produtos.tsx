...
export default function Produtos() {
  ...
  // Categorias predefinidas para classificação no Dashboard
  ...
  }, [produtoAtual.pesoPorUnidadeKg]); // Alterado para observar pesoPorUnidadeKg em vez de id
  ...
  const produtosFiltrados = produtos
    ...
    .sort((a, b) => {
      // Aplicar ordenação alfabética se ativada
      if (ordenacaoAlfabetica === "asc") {
        return a.nome.localeCompare(b.nome, "pt-BR");
      } else if (ordenacaoAlfabetica === "desc") {
        return b.nome.localeCompare(a.nome, "pt-BR");
      }
      // Sem ordenação, manter ordem original
      return 0;
    });
  ...
  const exportarExcel = () => {
    ...
    XLSX.writeFile(wb, `Produtos_${new Date().toLocaleDateString("pt-BR")}.xlsx`);
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  value={produtoAtual.responsavelProducao || "none"}
                  onValueChange={(valor) =>
                    setProdutoAtual({ 
                      ...produtoAtual, 
                      responsavelProducao: valor === "none" ? "" : valor as "Padeiro" | "Confeiteiro" | ""
                    })
                  }
                >
